<div x-data="composePage()" @compose-dirty.window="dirty = true" @compose-saved.window="dirty = false">
        ...
        <div class="bg-white border border-base-300 rounded-xl shadow-sm overflow-hidden">
            <div class="flex items-center justify-between px-4 py-3 bg-base-100 border-b border-base-300">
                ...
                <button wire:click="openCsvModal"
                    class="flex items-center gap-2 btn btn-xs btn-outline clr-bg-accent text-white hover-clr-bg-accent-light rounded-lg p-4">
            ...
            <div class="px-4 py-3 border-b border-base-300">
                <div class="flex items-center gap-2">
                    <input type="email" wire:model="manualEmail" wire:keydown.enter="addRecipient"
                    ...
                    <button wire:click="addRecipient" @click="$dispatch('compose-dirty')"
                        class="btn btn-sm clr-bg-accent text-white rounded-lg p-4 hover-clr-bg-accent-light">Add</button>
                </div>
                @if ($manualEmailError)
            ...
            <div class="px-4 py-3 min-h-[52px]">
                <div class="flex flex-wrap gap-2 items-center">
                    @if (empty($recipients))
                        <span class="text-sm text-gray-300 italic">No recipients yet. Type an email above or import a
                            CSV.</span>
                    @else
                        @foreach ($visibleRecipients as $email)
                            <div wire:key="chip-{{ $email }}" class="badge badge-info gap-1 text-xs py-3 px-2">
                                <span
                                    class="w-4 h-4 rounded-full bg-blue-700 text-white text-[9px] font-bold flex items-center justify-center">{{ strtoupper($email[0]) }}</span>
                                {{ $email }}
                        ...
                        @if (!$showAllRecipients && count($recipients) > 4)
                            <div wire:click="$set('showAllRecipients', true)"
                                class="badge badge-ghost cursor-pointer text-xs py-3">
                                +{{ count($recipients) - 4 }} more
                            </div>
                        @endif

                        @if ($showAllRecipients && count($recipients) > 4)
                            <div wire:click="$set('showAllRecipients', false)"
                                class="badge badge-ghost cursor-pointer text-xs py-3">
                                Show less
                            </div>
                        @endif
    ...
    {{-- Subject --}}
    <input type="text" wire:model="subject" @input="dirty = true" placeholder="Subject..."
    ...
    <div class="mb-4" x-data="richTextEditor({{ json_encode($body ?? '') }})">
        ...
        @endif
        <div
            class="bg-white border border-base-300 rounded-xl shadow-sm overflow-hidden focus-within:border-red-500 focus-within:shadow-md transition-all">

            {{-- Formatting toolbar --}}
            <div
                class="flex items-center flex-nowrap gap-2 px-3 py-2 bg-base-100 border-b border-base-300 overflow-x-auto">
                <button type="button" @mousedown.prevent="format('bold')"
                    class="btn btn-xs btn-ghost font-mono font-bold px-3 shrink-0">B</button>
                <button type="button" @mousedown.prevent="format('italic')"
                    class="btn btn-xs btn-ghost font-mono italic px-3 shrink-0">I</button>
                <button type="button" @mousedown.prevent="format('underline')"
                    class="btn btn-xs btn-ghost font-mono underline px-3 shrink-0">U</button>
                <button type="button" @mousedown.prevent="format('strikeThrough')"
                    class="btn btn-xs btn-ghost font-mono line-through px-3 shrink-0">S</button>
                <div class="w-px h-5 bg-base-300 mx-1 shrink-0 hidden sm:block"></div>
                <button type="button" @mousedown.prevent="format('insertUnorderedList')"
                    class="btn btn-xs btn-ghost px-3">
                ...
                <button type="button" @mousedown.prevent="format('insertOrderedList')"
                    class="btn btn-xs btn-ghost px-3">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2"
                        stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
                ...
                <button type="button" @mousedown.prevent="format('justifyCenter')"
                    class="btn btn-xs btn-ghost px-3">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2"
                        viewBox="0 0 24 24">
                        <line x1="3" y1="6" x2="21" y2="6" />
            ...
            {{-- Editable Area --}}
            <div wire:ignore>
                <div id="compose-editor-body" x-ref="editor" contenteditable="true"
                    data-placeholder="Start typing your message here..." @input="$dispatch('compose-dirty')"
                    ...
                    @foreach ($attachments as $i => $attachment)
                        <div class="badge badge-outline gap-1 text-xs flex items-center">
                            <a href="{{ $attachment->temporaryUrl() }}" target="_blank" rel="noopener"
                                class="hover:underline flex items-center gap-1" title="View file">
    ...
    @if (!empty($recipients))
        <div class="mb-4">
            ...
            <div class="bg-white border border-base-300 rounded-xl shadow-sm overflow-hidden">
                <div class="px-4 py-3 border-b border-base-300">
                    <input type="search" wire:model.live.debounce.300ms="recipientsSearch"
                            ...
                            <div x-data="{ expanded: false }" class="bg-white">
                                <div class="flex items-center justify-between gap-3 py-3 px-4">
                                    <div class="flex items-center gap-2 min-w-0 flex-1">
                                        <div
                                            class="w-7 h-7 shrink-0 rounded-full bg-blue-700 text-white text-xs font-bold flex items-center justify-center">
                                            {{ strtoupper($email[0]) }}
                                        </div>
                                    ...
                                    <button type="button" @click="expanded = !expanded"
                                        ...
                                        <svg x-show="!expanded" class="w-5 h-5 text-gray-500" fill="none"
                                            stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                            <path d="M19 9l-7 7-7-7" />
                                        </svg>
                                        <svg x-show="expanded" x-cloak class="w-5 h-5 text-gray-500" fill="none"
                                            stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                ...
                                <div x-show="expanded" x-transition class="px-4 pb-4 pt-0">
                                    <div class="flex items-center justify-between gap-3">
                                        <span
                                            class="badge badge-sm {{ $status === 'sent' ? 'badge-success' : ($status === 'failed' ? 'badge-error' : 'badge-warning') }}">
                                            {{ ucfirst($status) }}
                                        </span>
                                        <button wire:click="removeRecipient('{{ $email }}')"
                                            class="btn btn-xs btn-ghost hover:text-red-500 shrink-0">Remove</button>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                                ...
                                @foreach ($recipientsPaginator as $i => $email)
                                    <tr>
                                        <td class="text-xs text-gray-400 font-mono">
                                            {{ $recipientsPaginator->firstItem() + $i }}</td>
                                        <td>
                                            <div class="flex items-center gap-2 min-w-0">
                                                <div
                                                    class="w-7 h-7 rounded-full bg-blue-700 text-white text-xs font-bold flex items-center justify-center">
                                                    {{ strtoupper($email[0]) }}</div>
                                                <span
                                                    class="min-w-0 flex-1 text-sm whitespace-nowrap truncate">{{ $email }}</span>
                                            </div>
                                        </td>
                                        <td>
                                            @php $status = $recipientStatuses[$email] ?? 'pending'; @endphp
                                        ...
                                        <td>
                                            <button wire:click="removeRecipient('{{ $email }}')"
                                                class="btn btn-xs btn-ghost hover:text-red-500">×</button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
    ...
    {{-- CSV Modal --}}
    <div x-show="$wire.showCsvModal" x-transition
        ...
        <div class="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
            ...
            <div class="p-5">
                ...
                @if (empty($csvHeaders))
                    <label
                        class="border-2 border-dashed border-base-300 rounded-xl p-8 text-center cursor-pointer hover:border-red-400 hover:bg-red-50 transition-all mb-4 block">
                    ...
                    <div wire:loading wire:target="csvFile" class="text-center text-sm text-gray-400">Parsing CSV...
                    </div>
                @else
                    <div class="mb-4">
                        <label class="text-xs font-semibold text-gray-400 uppercase block mb-1">Which column contains
                            emails?</label>
                    ...
                    <div class="mb-4 p-3 rounded-lg bg-amber-50 border border-amber-200">
                        ...
                        <div class="flex flex-wrap gap-1.5">
                            @foreach ($csvHeaders as $header)
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-mono font-semibold bg-amber-100 text-amber-700 border border-amber-300">
                                    &#123;&#123;{{ $header }}&#125;&#125;
                                </span>
                            @endforeach
                        </div>
                        <p class="mt-1 text-[11px] text-amber-700">
                            Example: type <code
                                class="px-1 py-0.5 rounded bg-white border border-amber-200 font-mono text-[11px]">&#123;&#123;name&#125;&#125;</code>
                            in the editor and it will be replaced per row when you send.
                        </p>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="table table-xs w-full mb-4">
                            ...
                            <tbody>
                                ...
                                @if (count($csvRows) > 5)
                                    <tr>
                                        <td colspan="{{ count($csvHeaders) }}" class="text-gray-400 italic text-xs">
                                            ...and {{ count($csvRows) - 5 }} more rows</td>
                                    </tr>
                                @endif
                            </tbody>
                        </table>
            ...
            <div class="flex items-center justify-end gap-2 px-5 py-3 bg-base-100 border-t border-base-300">
                ...
                @if (!empty($csvHeaders) && $selectedEmailColumn !== '')
                    @php
                        $count = collect($csvRows)
                            ->filter(
                                fn($row) => !empty($row[(int) $selectedEmailColumn]) &&
                                    str_contains($row[(int) $selectedEmailColumn], '@'),
                            )
                            ->count();
                    @endphp
                    <button wire:click="importFromCsv" @click="$dispatch('compose-dirty')"
                        class="btn btn-sm clr-bg-accent text-white p-4">
...
{{-- Sending Modal --}}
@if ($showSendingModal)
    <div wire:key="sending-progress-modal" wire:poll.250ms="checkSendingProgress"
        class="fixed inset-0 bg-black/40 z-[9999] flex items-center justify-center">
        <div class="bg-white rounded-2xl px-10 py-8 text-center shadow-2xl w-full max-w-sm">
            <div class="w-10 h-10 border-4 border-base-200 border-t-red-500 rounded-full animate-spin mx-auto mb-4">
            </div>
            ...
            <div class="w-full bg-base-200 rounded-full h-2 mb-3">
                <div class="clr-bg-accent h-2 rounded-full transition-all duration-300"
                    style="width: {{ $sendTotal > 0 ? ($sendCurrent / $sendTotal) * 100 : 0 }}%"></div>
            </div>
        </div>
    </div>
@endif
